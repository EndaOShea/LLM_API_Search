...
Qwen has a related but distinct problem: DashScope's live `/models` catalog spans hundreds of non-frontier Qwen variants (sizes, VL, audio, coder, older generations), so `fetch_live_models()` is also curated-only, and its `unrecognized_live_model_ids()` override is pattern-filtered — it only flags live IDs shaped like a new frontier generation (`qwen{N}.{M}-max`/`-plus`, via a regex in `qwen.py`) rather than diffing the entire catalog, so the weekly signal doesn't drown in noise. Kimi's catalog is small and versioned, so it uses the same full-auto-discovery `fetch_live_models()` pattern as Z.ai/DeepSeek's non-curated peers, with no `unrecognized_live_model_ids()` override needed.
